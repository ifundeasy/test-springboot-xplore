...
docker network create --driver overlay --attachable test-sprintboot-xplore_default
...
docker run -d --rm --name eureka -p 8761:8761 \
  --network test-sprintboot-xplore_default \
  --health-cmd="curl -f http://localhost:8761/actuator/health || exit 1" \
  xplore/eureka-server:1.0.0
...
docker run -d --rm --name categorizer-service -p 9081:8080 \
  ...
  --network=test-sprintboot-xplore_default \
  --health-cmd="curl -f http://localhost:8080/actuator/health || exit 1" \
  xplore/categorizer-service:1.0.0
...
docker run -d --rm --name collector-service -p 9082:8080 \
  ...
  --network=test-sprintboot-xplore_default \
  --health-cmd="curl -f http://localhost:8080/actuator/health || exit 1" \
  xplore/collector-service:1.0.0
